...
contract Lottery is Ownable {
    ...
    // Last winner
    address private _lastWinner = address(0);
  ...
  /*
    ...
    1. Название событий всегда с большой буквы ✔
    2. События всегда идут в конце функции перед return, и только в том случае если функция выполнилась успешно ✔
    3. Еще раз посмотри правильность написания описания функций(немного не так)✔
    4. В последей фугкции не обязательно создавать переменные а потом возвращать их, это лишнее . ✔
    5. Не описаны все функции(Желательно описывать все функции, что они делают и что возвращают)✔
    6. Проблемы с безопасностью (Любой может вызвать некоторые функции, ограничь права, а где именно сам найди)✔
    7.Переменные в параметрах функции задавай без "_", а переменные внутри контракта с "_"(это для единого стиля)✔
    8.Особое внимание ограничений по правам!!!!!!У тебя с этим проблемы есть в контракте✔
