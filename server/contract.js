...
const payWinning = async () => {
  try {const lotteryIsActive = await contract.methods.getLotteryIsActive().call();
  if(!lotteryIsActive) {
    await contract.methods.pay().send({from: address});
    console.log("Winning payed");
...
const restart = async () => {
  try {const lotteryIsActive = await contract.methods.getLotteryIsActive().call();
  if(!lotteryIsActive) {
    await contract.methods.restart().send({from: address});
...
const getAllInfo = async () => {
    return Promise.all([contract.methods.getAdressOfMembers().call(), contract.methods.getLastWiner().call(), contract.methods.getLotteryIsActive().call(), contract.methods.owner().call(), contract.methods.getTicketPrise().call(), contract.methods.getLotteryDuration().call(), contract.methods.getUpToTime().call(), contract.methods.getMaxPlayers().call(), contract.methods.getMembersLength().call()]).then(res => {
      return {
        name: "Lottery",
